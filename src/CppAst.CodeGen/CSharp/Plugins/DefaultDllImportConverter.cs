...
public class DefaultDllImportConverter : ICSharpConverterPlugin
{
    ...
    protected virtual void AddDefaultDllImport(CSharpConverter converter, CSharpElement element, CSharpElement context)
    {
        ...
        var dllImportName = converter.GetDefaultDllImportName(element.CppElement!, element);

        if (converter.Options.UseLibraryImport)
        {
            method.Modifiers &= ~CSharpModifiers.Extern;
            method.Modifiers |= CSharpModifiers.Partial;
            method.Attributes.Add(new CSharpLibraryImportAttribute(dllImportName) { EntryPoint = $"\"{method.Name}\"" });
        ...
        {
            method.Attributes.Add(new CSharpDllImportAttribute(dllImportName) { CallingConvention = csCallingConvention });
        }
    }
}