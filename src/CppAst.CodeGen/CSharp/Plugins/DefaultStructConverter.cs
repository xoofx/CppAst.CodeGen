...
public class DefaultStructConverter : ICSharpConverterPlugin
{
    ...
    public static CSharpElement? ConvertClass(CSharpConverter converter, CppClass cppClass, CSharpElement context)
    {
        ...
        if (cppClass.ClassKind != CppClassKind.Struct && cppClass.ClassKind != CppClassKind.Union)
        {
            return null;
        }
        ...
        var structLayout = new CSharpStructLayoutAttribute(isUnion && cppClass.IsDefinition ? LayoutKind.Explicit : LayoutKind.Sequential);
        ...
        if (!cppClass.IsDefinition && cppClass.Fields.Count == 0)
        {
            ...
            if (converter.Options.DetectOpaquePointers)
            {
                ...
                csStruct.PrimaryConstructorParameters.Add(new CSharpParameter("Handle") { ParameterType = CSharpPrimitiveType.IntPtr()});
