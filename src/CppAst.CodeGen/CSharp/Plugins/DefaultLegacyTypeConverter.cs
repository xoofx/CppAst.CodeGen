...
namespace CppAst.CodeGen.CSharp
{
    ...
    public class DefaultLegacyTypeConverter : ICSharpConverterPlugin
    {
                ...
                switch (cppType.TypeKind)
                {
                    case CppTypeKind.Array:
                        ...
                        else if (arrayType.Size > 0 && (arrayElementType.Equals(CppPrimitiveType.Char) || arrayElementType.Equals(CppPrimitiveType.WChar)))
                        {
                            ...
                            fixedStrType.Attributes.Add(new CSharpMarshalAsAttribute(UnmanagedType.ByValTStr) { SizeConst = arrayType.Size });
                            csType = fixedStrType;
                        }
                        else
                        {
                            ...
                            else
                            {
                                ...
                                var attr = new CSharpMarshalAsAttribute(UnmanagedType.LPArray);
        ...
        private static CSharpMarshalAsAttribute? GetMarshalAttributeOrNull(List<CSharpAttribute> attributes)
        {
            foreach (var cSharpAttribute in attributes)
            {
                if (cSharpAttribute is CSharpMarshalAsAttribute csMarshalAttribute)
                {
